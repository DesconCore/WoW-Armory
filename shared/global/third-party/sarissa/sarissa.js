 ...
 */
function Sarissa(){}
Sarissa.VERSION = "0.9.9.7 (modified by Blizzard Entertainment)";
    ...
    if(Sarissa._SARISSA_HAS_DOM_CREATE_DOCUMENT){
        ...
        */
		...
		oDoc.load = function(url) {
			...
			xmlhttp.open("POST", url, oDoc.async);
			xmlhttp.send();
        ...
        if(ownerDoc.parseError && ownerDoc.parseError.errorCode != 0) {
            ...
            if(oTargetElement.tagName.toLowerCase() == "textarea" || oTargetElement.tagName.toLowerCase() == "input") {
                ...
                try{
					...
					// Opera 9 includes xml PI in output.  See http://codespeak.net/pipermail/kupu-dev/2006-June/001891.html
					if(Sarissa._SARISSA_IS_OPERA) {
						for(var i = 0, c; c = oNode.childNodes[i]; i++)
							oTargetElement.innerHTML += new XMLSerializer().serializeToString(oNode.childNodes[i]);
					} else
